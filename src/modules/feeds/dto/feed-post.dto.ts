...
export const PostMediaInputSchema = z.object({
  url: z.string({ required_error: 'Media URL is required' }).min(1, 'Media URL cannot be empty'),
...
export const CreatePostDtoSchema = z
  .object({
    ...
    media: z.array(PostMediaInputSchema).max(10, 'Maximum 10 media items allowed per post').optional().default([]),
  })
  .refine(
    (data) => {
      // If no destinationId is provided, location (name, latitude, longitude) is required
      if (!data.destinationId) {
        return !!data.location && !!data.location.name && typeof data.location.latitude === 'number' && typeof data.location.longitude === 'number';
      }
      return true;
    },
    {
...
export const UpdatePostDtoSchema = z.object({
  ...
  media: z.array(PostMediaInputSchema).max(10, 'Maximum 10 media items allowed per post').optional(),
});
